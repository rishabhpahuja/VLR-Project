...
def matching_cascade(distance_metric, max_distance, cascade_depth, tracks, detections, frame_t, frame_t_1,
        track_indices=None, detection_indices=None):
    """Run matching cascade.
    ...
    for level in range(cascade_depth): #casade_depth - max_age "of" tracks
        ...
        matches_l, _, unmatched_detections = \
            min_cost_matching(
                distance_metric, max_distance, tracks, detections,
                track_indices_l, unmatched_detections, frame_t, frame_t_1)
        matches += matches_l
    unmatched_tracks = list(set(track_indices) - set(k for k, _ in matches))
    return matches, unmatched_tracks, unmatched_detections


def min_cost_matching(distance_metric, max_distance, tracks, detections, frame_t, frame_t_1, 
            track_indices=None,detection_indices=None):
    """Solve linear assignment problem.
    ...
    # ipdb.set_trace()
    cost_matrix = distance_metric(tracks, detections, frame_t, frame_t_1, track_indices, detection_indices) # calls gated_metric - tracks x detctions
    cost_matrix[cost_matrix > max_distance] = max_distance + 1e-5 # max distance is 0.4
    indices = linear_sum_assignment(cost_matrix) #Link the detection to tracker
    indices = np.asarray(indices) #Makes a 2D array with row at indices at row 0 and column indices at row 1
    indices = np.transpose(indices) #Convert it in the form of (row,column)
